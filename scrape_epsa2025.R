...
df_long <- df %>%
  ...
  # Keep only matching author/affil pairs (e.g., author1 with author1_affil)
  filter(
...
(df_long$author_affil %>% table %>% sort(decreasing = T))[1:20]



# Optional: drop the author_num and affil_num columns if you don't need them
df_long <- df_long %>%
...
(html_files_to_parse[17] %>% parse_abstract_html)$affil1
